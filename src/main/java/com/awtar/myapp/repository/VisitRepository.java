...
public interface VisitRepository extends JpaRepository<Visit, Long> {
    ...
    @Query(
        value = "select distinct visit from Visit visit left join fetch visit.beneficiary",
        countQuery = "select count(distinct visit) from Visit visit"
    )
    Page<Visit> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct visit from Visit visit left join fetch visit.beneficiary")
    List<Visit> findAllWithToOneRelationships();

    @Query("select visit from Visit visit left join fetch visit.beneficiary where visit.id =:id")
