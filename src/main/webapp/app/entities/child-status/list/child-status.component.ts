...
export class ChildStatusComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.childStatusService
      ...
      .subscribe({
        next: (res: HttpResponse<IChildStatus[]>) => {
  ...
  trackId(_index: number, item: IChildStatus): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IChildStatus[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/child-status'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
