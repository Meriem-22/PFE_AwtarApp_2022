...
export class EducationalInstitutionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.educationalInstitutionService
      ...
      .subscribe({
        next: (res: HttpResponse<IEducationalInstitution[]>) => {
  ...
  trackId(_index: number, item: IEducationalInstitution): number {
    return item.id!;
  }

  delete(educationalInstitution: IEducationalInstitution): void {
    const modalRef = this.modalService.open(EducationalInstitutionDeleteDialogComponent, { size: 'lg', backdrop: false, keyboard: false });
  ...
  protected onSuccess(data: IEducationalInstitution[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/educational-institution'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
