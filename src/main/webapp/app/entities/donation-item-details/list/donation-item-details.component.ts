...
@Component({
  selector: 'jhi-donation-item-details',
...
export class DonationItemDetailsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.donationItemDetailsService
      .query({
        page: pageToLoad - 1,
  ...
  trackId(_index: number, item: IDonationItemDetails): number {
    return item.id!;
  }

  delete(donationItemDetails: IDonationItemDetails): void {
    const modalRef = this.modalService.open(DonationItemDetailsDeleteDialogComponent, { size: 'lg', backdrop: false, keyboard: false });
    modalRef.componentInstance.donationItemDetails = donationItemDetails;
  ...
  protected onSuccess(data: IDonationItemDetails[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
