...
export class DonationsReceivedComponent implements OnInit {
  donationsReceiveds?: IDonationsReceived[];
  isLoading = false;
  totalItems = 0;
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.donationsReceivedService
      ...
      .subscribe({
        next: (res: HttpResponse<IDonationsReceived[]>) => {
  ...
  delete(donationsReceived: IDonationsReceived): void {
    const modalRef = this.modalService.open(DonationsReceivedDeleteDialogComponent, { size: 'lg', backdrop: false, keyboard: false });
    modalRef.componentInstance.donationsReceived = donationsReceived;
  ...
  protected onSuccess(data: IDonationsReceived[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/donations-received'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
